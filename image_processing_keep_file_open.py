...
def stream_video_in_chunks(filename, step_size=1000, grayscale=True):
    cap = cv2.VideoCapture(filename)
    if not cap.isOpened():
    ...
    while True:
        ...
        if frame_idx % step_size == 0 or frame_idx == total_frames:
            yield np.array(current_chunk, dtype=np.uint8)
            current_chunk = []
...
def main(
    ...
    for idx, chunk in enumerate(stream_video_in_chunks(video_filename, step_size=step_size)):
        if idx >= max_chunks:
            break
        print(f"Processing chunk {idx}")
        counts, clusters, sizes, brightness = detect_damaged_pixels(chunk, settings, plot=False)
        frames_count.append(counts)
        all_clusters.append(clusters)
        all_sizes.append(sizes)
        all_brightness.append(brightness)
