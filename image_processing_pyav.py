...
def load_video_frames(filename, frames_start=None, frames_end=None, grayscale=True):
    """
    Streams video frames lazily using the pyav backend, avoiding FFmpeg subprocesses.
    Ensures consistent (n, h, w) array output and skips empty or malformed chunks.
    """
    frames = []

    try:
        with iio.imopen(filename, io_mode="r", plugin="pyav") as file:
            for i, frame in enumerate(file.iter()):
                ...
                frames.append(np.asarray(frame, dtype=DTYPE_IMAGE))

    except Exception as e:
        print(f"[WARNING] Frame loading interrupted in {filename}: {e}")

    # --- Safeguards ---
    if not frames:
        print(f"[WARNING] No frames loaded from {filename} between {frames_start}-{frames_end}")
        # return an empty 3D array so downstream code can handle it
        return np.empty((0, 0, 0), dtype=DTYPE_IMAGE)

    # Ensure consistent shape (some codecs give slightly different sizes)
    frame_shapes = {f.shape for f in frames}
    if len(frame_shapes) > 1:
        print(f"[WARNING] Inconsistent frame shapes {frame_shapes}. Resizing all to first frame shape.")
        h, w = frames[0].shape
        frames = [cv2.resize(f, (w, h)) for f in frames]

    return np.stack(frames, axis=0).astype(DTYPE_IMAGE)
