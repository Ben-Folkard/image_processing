...
def _generate_plots(
    ...
    heatmap = find_damaged_pixel_heatmap(
        frames,
        [m.astype(DTYPE_IMAGE) for m in masks if m is not None]
    )
    plot_heatmap(
                 heatmap,
                 show_plots=show_plots,
...
def compute_static_mask(
    ...
    filled_masks = [(mask if mask is not None else np.zeros(mask_shape, dtype=DTYPE_MASK)) for mask in masks]

    mask_stack = np.stack([mask.astype(DTYPE_IMAGE) for mask in filled_masks], axis=0)
...
def estimate_damaged_pixels_in_bright_areas(
    ...
    # preprocess masks
    processed_masks = np.zeros((num_frames, *frame_shape), dtype=DTYPE_MASK)
